...
class BroadcastSearchRequest {
    ...
    static let TAG = String(describing: BroadcastSearchRequest.self)
    ...
    func createQuery(_ urlQueryItems: [URLQueryItem]) {
        ...
        if let url = URL(string: components.string ?? Configuration.API_URL_SEARCH) {
            GeneralUtils.log(BroadcastSearchRequest.TAG, url)
            GeneralUtils.log(BroadcastSearchRequest.TAG, urlQueryItems)
