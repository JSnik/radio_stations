...
class UIButtonOctonary: UIButtonBaseNotBold {
    ...
    @IBInspectable var largeMode: Bool = true {
        didSet {
            updateFont()
...
//        constraintHeight?.constant = CGFloat(40)
    ...
    func updateFont() {
        if (largeMode) {
            ...
            attributeContainer.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont4 ?? UIFont.systemFont(ofSize: 16.0))
            //UIFont(name: FontsHelper.FUTURA_PT_DEMI, size: 16.0)
//            if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {
//
//                attributeContainer.font = UIFont(descriptor: artistDescriptor,
//                                              size: 14.0)
//                }
        } else {
            let customFont4 = UIFont(name: FontsHelper.FUTURA_PT_MEDIUM, size: 14.0)
            attributeContainer.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont4 ?? UIFont.systemFont(ofSize: 14.0))
            //UIFont(name: FontsHelper.FUTURA_PT_MEDIUM, size: 14.0)
//            if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {
//
//                attributeContainer.font = UIFont(descriptor: artistDescriptor,
//                                              size: 14.0)
