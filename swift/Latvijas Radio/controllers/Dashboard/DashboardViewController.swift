...
class DashboardViewController: UIViewController, UIScrollViewDelegate {
    ...
    static var livestreamsCompactListNeedsUpdate = true
    ...
    var fullRadioChannelDataset: [RadioChannel]! = [RadioChannel]()
    ...
    override func viewDidLoad() {
        ...
        let customFont = UIFont(name: "FuturaPT-Bold", size: 20.0)
...
//                var fullRadioChannelDataset: [RadioChannel]!
                ...
                if let currentUser = usersManager.getCurrentUser() {
                    let orderedLivestreamsDataset = LivestreamsManager.getOrderedList(currentUser, fullRadioChannelDataset)
                    GeneralUtils.log(DashboardViewController.TAG, "1 orderedLivestreamsDataset.count = \(orderedLivestreamsDataset.count)")
                    let curatedLivestreamsDataset = LivestreamsManager.getCuratedList(orderedLivestreamsDataset, isChristmasLivestreamCampaignEnabled /*, fullRadioChannelDataset*/)
                    GeneralUtils.log(DashboardViewController.TAG, "1 curatedLivestreamsDataset.count = \(curatedLivestreamsDataset.count)")
                    ...
                    if let r1 = saveJsonFromData(data: data1, name: "radioChannels1") as? Bool,
                       r1 == true {
                        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                            let orderedLivestreamsDataset = LivestreamsManager.getOrderedList(currentUser, self.fullRadioChannelDataset)
                            let curatedLivestreamsDataset = LivestreamsManager.getCuratedList(orderedLivestreamsDataset, self.isChristmasLivestreamCampaignEnabled /*, self.fullRadioChannelDataset*/)
                            GeneralUtils.log(DashboardViewController.TAG, "2 curatedLivestreamsDataset.count = \(curatedLivestreamsDataset.count)")
                            self.livestreamsCollectionViewController.updateDataset(curatedLivestreamsDataset)
                        }
//                        livestreamsCollectionViewController.updateDataset(curatedLivestreamsDataset)
                    }
...
//                        reload data ui
                        self.livestreamsCollectionViewController.collectionView.reloadData()
                    }
                } catch {
