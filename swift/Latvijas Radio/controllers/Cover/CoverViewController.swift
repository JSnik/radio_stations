...
class CoverViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let customFont = UIFont(name: "FuturaPT-Bold", size: 10.0) ?? UIFont.systemFont(ofSize: 10.0) 
//        textBroadcastName.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont)
        if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {

            textBroadcastName.font = UIFont(descriptor: artistDescriptor,
                                size: 10.0)
        }
        textBroadcastName.adjustsFontForContentSizeCategory = true
        let customFont1 = UIFont.systemFont(ofSize: 13.0) // UIFont(name: "FuturaPT-Bold", size: 13.0) //UIFont.systemFont(ofSize: 13.0)
//        textEpisodeTitle.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont1)
        if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {

            textEpisodeTitle.font = UIFont(descriptor: artistDescriptor,
                                size: 13.0)
        }
