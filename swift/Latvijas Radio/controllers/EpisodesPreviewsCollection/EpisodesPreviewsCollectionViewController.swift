...
extension EpisodesPreviewsCollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {
            ...
            cell.textBroadcastTitle.font = UIFont(descriptor: artistDescriptor,
                                size: 10.0)
        }
        cell.textBroadcastTitle.adjustsFontForContentSizeCategory = true
        let customFont1 = UIFont.systemFont(ofSize: 13.0)//UIFont(name: "FuturaPT-Bold", size: 13.0)
        cell.textEpisodeTitle.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont1 ?? UIFont.systemFont(ofSize: 13.0))
        if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {

            cell.textEpisodeTitle.font = UIFont(descriptor: artistDescriptor,
                                size: 13.0)
        }
