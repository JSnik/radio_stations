...
class EpisodesCollectionViewController: UICollectionViewController {
    ...
    private var currentPressGesture: UITapGestureRecognizer!
    ...
    override func viewDidLoad() {
        ...
        currentPressGesture = UITapGestureRecognizer(target: self, action: #selector(self.handleGesture(gesture:)))
        collectionView.addGestureRecognizer(currentPressGesture)
    }
...
extension EpisodesCollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (isCurrentItemLoadMore) {
            ...
            let customFont1 = UIFont.systemFont(ofSize: 10.0) //UIFont(name: "FuturaPT-Bold", size: 10.0)
            cell.textDate.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont1)
            cell.textDate.adjustsFontForContentSizeCategory = true
            let customFont2 =  UIFont(name: "FuturaPT-Bold", size: 13.0) ??  UIFont.systemFont(ofSize: 13.0)
            cell.textTitle.font = UIFontMetrics(forTextStyle: .headline).scaledFont(for: customFont2)
            if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {

                cell.textTitle.font = UIFont(descriptor: artistDescriptor,
                                             size: 13.0)
            }
