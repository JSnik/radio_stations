...
extension LivestreamsCollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        GeneralUtils.log(LivestreamsCollectionViewController.TAG, "numberOfItemsInSection dataset.count = \(dataset.count)")
...
//        let wideImageResourceId = livestreamModel.getWideImageResourceId()
        ...
        if livestreamModel.name == "Radioteātris" {
            ...
            if (livestreamModel.image != nil) {
                cell.imageLivestream.sd_setImage(with: URL(string: livestreamModel.mobile?.wide_image ?? ""))
            } else {
        ...
        if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {
            ...
            cell.textTitlePrimary.font = UIFont(descriptor: artistDescriptor,
                                          size: 10.0)
            }
        cell.textTitlePrimary.adjustsFontForContentSizeCategory = true
        let customFont1 = UIFont(name: "FuturaPT", size: 13.0)
        ...
        if let artistDescriptor = UIFontDescriptor.preferredFontDescriptor(withTextStyle: .body).withSymbolicTraits(.traitBold) {
            ...
            cell.textTitleSecondary.font = UIFont(descriptor: artistDescriptor,
                                                 size: 13.0)
        }
