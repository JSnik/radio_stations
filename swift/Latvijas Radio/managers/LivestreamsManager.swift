...
class LivestreamsManager {
    ...
    private static var livestreamsInternetChannels: [RadioChannel]! //[LivestreamModel]!


    static func getLivestreamsClassic( _ allLivestreams1: [RadioChannel]) -> [RadioChannel] /*[LivestreamModel]*/ {
        if (LivestreamsManager.livestreamsClassic == nil) {
            LivestreamsManager.livestreamsClassic =  [RadioChannel]() //[LivestreamModel]()
//
//            // Latvijas Radio 1
...
//            LivestreamsManager.livestreamsClassic.append(livestreamModel)
        ...
        //            let json4Swift_Base = try SearchSuccess(someDictionaryFromJSON)
                    ...
                    if (radioChannels?.count ?? 0 > 0) {
                        ...
                        GeneralUtils.log(LivestreamsManager.TAG, "1 dataset.count = \(dataset.count)")
                        if let  fmClassic = radioChannels?.filter({ $0.is_fm == true}) as? [RadioChannel] {
                            LivestreamsManager.livestreamsClassic = fmClassic
                            GeneralUtils.log(LivestreamsManager.TAG, "1 LivestreamsManager.livestreamsClassic.count = \(LivestreamsManager.livestreamsClassic.count)")
    //                        allLivestreams.append(contentsOf: fmClassic)
    ...
    //            var allLivestreams = [RadioChannel]()
                GeneralUtils.log(LivestreamsManager.TAG, "2 LivestreamsManager.livestreamsClassic.count = \(LivestreamsManager.livestreamsClassic.count)")
                return LivestreamsManager.livestreamsClassic
            }
        }
//        
//        return LivestreamsManager.livestreamsClassic
//        var allLivestreams = [RadioChannel]()
        GeneralUtils.log(LivestreamsManager.TAG, "3 LivestreamsManager.livestreamsClassic.count = \(LivestreamsManager.livestreamsClassic.count)")
        if allLivestreams1.count > 0,
           let  fmClassic = allLivestreams1.filter({ $0.is_fm == true}) as? [RadioChannel] {
            LivestreamsManager.livestreamsClassic = fmClassic
            GeneralUtils.log(LivestreamsManager.TAG, "4 LivestreamsManager.livestreamsClassic.count = \(LivestreamsManager.livestreamsClassic.count)")
//                        allLivestreams.append(contentsOf: fmClassic)
            return LivestreamsManager.livestreamsClassic
        }
        if dataset.count > 0,
           let  fmClassic = dataset.filter({ $0.is_fm == true}) as? [RadioChannel] {
            LivestreamsManager.livestreamsClassic = fmClassic
            GeneralUtils.log(LivestreamsManager.TAG, "4 LivestreamsManager.livestreamsClassic.count = \(LivestreamsManager.livestreamsClassic.count)")
//                        allLivestreams.append(contentsOf: fmClassic)
            return LivestreamsManager.livestreamsClassic
    ...
    static func getLivestreamsInternetChannels( _ allLivestreams1: [RadioChannel]) -> [RadioChannel] /*[LivestreamModel]*/ {
        if (LivestreamsManager.livestreamsInternetChannels == nil) {
            LivestreamsManager.livestreamsInternetChannels = [RadioChannel]()  //[LivestreamModel]()
/*
            // Pieci Ziemassvētki
 ...
 */
        ...
        //            let json4Swift_Base = try SearchSuccess(someDictionaryFromJSON)
                    ...
                    if (radioChannels?.count ?? 0 > 0) {
                        ...
                        GeneralUtils.log(LivestreamsManager.TAG, "lives LivestreamsManager.dataset.count = \(LivestreamsManager.dataset.count)")
                        if let  fmClassic = radioChannels?.filter({ $0.is_fm == false}) as? [RadioChannel] {
                            LivestreamsManager.livestreamsInternetChannels = fmClassic
                            GeneralUtils.log(LivestreamsManager.TAG, "1 LivestreamsManager.livestreamsInternetChannels.count = \(LivestreamsManager.livestreamsInternetChannels.count)")
    //                        allLivestreams.append(contentsOf: fmClassic)
                ...
                print(error.localizedDescription)
    //            var allLivestreams = [RadioChannel]()
                GeneralUtils.log(LivestreamsManager.TAG, "2 LivestreamsManager.livestreamsInternetChannels.count = \(LivestreamsManager.livestreamsInternetChannels.count)")
                return LivestreamsManager.livestreamsInternetChannels
            }

        }
        GeneralUtils.log(LivestreamsManager.TAG, "3 LivestreamsManager.livestreamsInternetChannels.count = \(LivestreamsManager.livestreamsInternetChannels.count)")
        if allLivestreams1.count > 0,
           let  fmClassic = allLivestreams1.filter({ $0.is_fm == false}) as? [RadioChannel] {
            LivestreamsManager.livestreamsInternetChannels = fmClassic
            GeneralUtils.log(LivestreamsManager.TAG, "1 LivestreamsManager.livestreamsInternetChannels.count = \(LivestreamsManager.livestreamsInternetChannels.count)")
        }
        if dataset.count > 0,
           let  fmClassic = dataset.filter({ $0.is_fm == false}) as? [RadioChannel] {
            LivestreamsManager.livestreamsInternetChannels = fmClassic
            GeneralUtils.log(LivestreamsManager.TAG, "1 LivestreamsManager.livestreamsInternetChannels.count = \(LivestreamsManager.livestreamsInternetChannels.count)")
        }
        return LivestreamsManager.livestreamsInternetChannels
    ...
    static func getLivestreamByIdFromAllChannels(_ lookUpLivestreamId: String /*, _ allLivestreams1: [RadioChannel]*/) -> RadioChannel?  /*LivestreamModel?*/ {
        ...
        allLivestreams.append(contentsOf: getLivestreamsClassic(dataset))
        allLivestreams.append(contentsOf: getLivestreamsInternetChannels(dataset))
...
/*
    ...
    //            let json4Swift_Base = try SearchSuccess(someDictionaryFromJSON)
                ...
                if (radioChannels?.count ?? 0 > 0) {
                    ...
                    var allLivestreams = [RadioChannel]()
                    if let  fmClassic = radioChannels?.filter({ $0.is_fm == true}) {
                        allLivestreams.append(contentsOf: fmClassic)
                    }
                    if let  internetChannels = radioChannels?.filter({ $0.is_fm == false}) {
                        allLivestreams.append(contentsOf: internetChannels)
                    }
...
//                    allLivestreams.append(contentsOf: dataset)
        ...
        var finalLivestreamsDataset: [RadioChannel]!
        if (allLivestreams1.count > 0) {
            dataset = allLivestreams1
        }
        ...
        if let livestreamIds = currentUser.getLivestreamsOrder() {
            // Show livestreams in user specifier order.
            var allLivestreamsInUserSpecifierOrder = [RadioChannel]()

            for livestreamId in livestreamIds {
                // The livestream might be null (not found), because it is not supported by app anymore.
                if let livestreamModel = getLivestreamByIdFromAllChannels(livestreamId) {
                    allLivestreamsInUserSpecifierOrder.append(livestreamModel)
                }
            }
     ...
     */
    static func getCuratedList(_ livestreams: [RadioChannel] /*[LivestreamModel]*/, _ isChristmasLivestreamCampaignEnabled: Bool /*, _ allLivestreams1: [RadioChannel]*/ ) -> [RadioChannel] /*[LivestreamModel]*/ {
        var result = livestreams
        
        if let livestreamModel = getLivestreamByIdFromAllChannels(ID_PIECI_ZIEMASSVETKI) /*!*/ {
